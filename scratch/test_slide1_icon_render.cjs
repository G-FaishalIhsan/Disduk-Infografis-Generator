...
const payload = {
    html: slide1Html,
    filename: 'fixed_social_slide1.png',
    options: {
        width: 1080,
        height: 1350,
        theme: 'light',
...
const req = http.request(options, (res) => {
    ...
    res.on('end', () => {
        ...
        const outputPath = path.join(artifactDir, 'fixed_social_slide1.png');
        fs.writeFileSync(outputPath, buffer);
        console.log(`SUCCESS! Saved exported slide 1 image (${buffer.length} bytes) to: ${outputPath}`);
    });
});
