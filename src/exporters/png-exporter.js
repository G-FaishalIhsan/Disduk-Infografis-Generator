/**
 * PNG Exporter Driver Module (Phase 9 — Social Export Engine V2)
 * Renders active slide or specified HTML to a 1080x1350 px high-res PNG image with white background.
 * Receives ONLY ExportContext and executes backend Puppeteer capture.
 */

export class PngExporter {
    /**
     * Executes PNG export for the active slide or target slide index
    ...
    async export(exportContext, slideIndex = 1) {
        ...
        const filename = exportContext.filenames.png[idx] || `slide_${slideIndex}.png`;
        ...
        if (!response.ok) {
            ...
            throw new Error(errJson.error || `Server PNG render error (${response.status})`);
        }
